...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        ...
        try:
            ...
            self.app.show_info(_("Not a VIPSTARCOIN URI"))
            return
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a VIPSTARCOIN address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid VIPSTARCOIN Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share VIPSTARCOIN Request"))

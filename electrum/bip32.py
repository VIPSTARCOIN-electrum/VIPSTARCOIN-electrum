...
from .util import bfh, bh2u, VIPSTARCOINException, print_error
...
def _CKD_priv(k, c, s, is_prime):
    try:
        ...
        raise VIPSTARCOINException('Impossible xprv (not within curve order)') from e
...
def serialize_xprv(xtype, c, k, depth=0, fingerprint=b'\x00'*4,
    ...
    if not ecc.is_secret_within_curve_range(k):
        raise VIPSTARCOINException('Impossible xprv (not within curve order)')
...
class InvalidMasterKeyVersionBytes(VIPSTARCOINException): pass
...
def deserialize_xkey(xkey, prv, *, net=None):
    ...
    if len(xkey) != 78:
        raise VIPSTARCOINException('Invalid length for extended key: {}'
                               .format(len(xkey)))
    depth = xkey[4]
